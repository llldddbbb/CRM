	...
	var url;
 	...
 	function openDataDicModifyDialog(){
 		...
 		if(selectedRows.length!=1){
 			$.messager.alert("系统提示","清选择一条要编辑的数据！");
 			return;
 		}
 	...
 	function saveDataDic(){
 		...
 		if(dataDicName==null||dataDicName==""){
 			$.messager.alert("系统提示","数据字典名称不能为空!");
 			return ;
 		}
 		if(dataDicValue==null||dataDicValue==""){
 	...
 	function deleteDataDic(){
 		...
 		$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗?",function(r){
 			if(r){
 				...
 				var strIds=ids.join(",")
 				$.post("${pageContext.request.contextPath}/dataDic/delete.do",{ids:strIds},function(result){
 					var result=eval("("+result+")");
 					if(result.success){
...
<title>数据字典信息查询</title>
</head>
<body style="margin: 1px">
	<table id="dg" title="数据字典管理" class="easyui-datagrid" fitColumns="true" pagination="true" 
	rownumbers="true" url="${pageContext.request.contextPath}/dataDic/list.do" fit="true" toolbar="#tb">
	<thead>
		<tr>
	...
	<div id="tb">
		...
		<div> 
			&nbsp;数据字典名：&nbsp;
			<input class="easyui-combobox" id="s_dataDicName" data-options="panelHeight:'auto',editable:false,valueField:'dataDicName',textField:'dataDicName',url:'${pageContext.request.contextPath}/dataDic/findDataDicName.do'"/>
			&nbsp;数据字典值：&nbsp;
			<input type="text" id="s_dataDicValue" onkeyup="if(event.keyCode==13) searchDataDic()"/>
			<a href="javascript:searchDataDic()" class="easyui-linkbutton" iconCls="icon-search" plain="true">查询</a>
		</div>
	</div>
	<div id="dlg" class="easyui-dialog" style="width:620px;height:150px; padding:10px 20px" buttons="#dlg-buttons" closed="true">
		<form id="fm" method="post">
			<table cellspacing="8px">
				<tr>
					<td>数据字典名称</td>
					<td><input class="easyui-combobox" id="dataDicName" name="dataDicName" data-options="panelHeight:'auto',valueField:'dataDicName',textField:'dataDicName',url:'${pageContext.request.contextPath}/dataDic/findDataDicName.do'"/></td>
					<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
					<td>数据字典值</td>
	...
	<div id="dlg-buttons">
		<a href="javascript:saveDataDic()" class="easyui-linkbutton" iconCls="icon-add">保存</a>
